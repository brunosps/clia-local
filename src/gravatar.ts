// Minimal MD5 (public-domain blueimp-md5 core, condensed) — only used to build
// Gravatar URLs from author emails. No runtime dependency.
...
function bytesToWords(input: string): number[] {
  const words: number[] = [];
...
function md5cycle(input: string): number[] {
...
export function md5(input: string): string {
  // UTF-8 encode so non-ASCII emails hash correctly.
  const utf8 = unescape(encodeURIComponent(input));
  return md5cycle(utf8).map(wordToHex).join("");
}

/** Gravatar URL for an email; `d=404` so a missing avatar errors → initials fallback. */
